...
/* methods  */
	...
	SetData: function(DATA, current_data_id, page_number, page_count)
	{
		...
		for (var i = 0, dateTo; i < DATA.length; i++)
		{
			if (null != DATA[i].DATA)
			{
				...
				{
					DATA[i].DATA[j].DATE_ACTIVE_FROM = BX.parseDate(DATA[i].DATA[j].ACTIVE_FROM);
					DATA[i].DATA[j].DATE_ACTIVE_TO   = BX.parseDate(DATA[i].DATA[j].ACTIVE_TO);
	...
	RegisterEntry: function(arEntry)
	{
		...
		if (null != arEntry.VISUAL)
		{
			...
			this._INFO_CACHE[arEntry.ID].CLIENT_ID = arEntry.CLIENT_ID
			//append Hint
			var hintContent = '';
			hintContent += arEntry.ACTIVE_FROM + ' - ' + arEntry.ACTIVE_TO + '<br />';
			var desc_len = arEntry.CLIENT_NAME?.length, max_len = 350;
			if (desc_len > 0)
			{
				if (desc_len < max_len)
					hintContent += '<br>' + arEntry.CLIENT_NAME;
				else
					hintContent += '<br>' + arEntry.CLIENT_NAME.substr(0, max_len) + '...';
			}
			hintContent += '<br>' + arEntry.MODEL_VALUE;
			this.DEAL_ID = arEntry.DEAL_ID
...
JCCalendarInfoWin.prototype.Show = function(e)
{
	...
	if (null == this.DIV)
	{
		var clientId = Number(this.CLIENT_ID)
		BX.SidePanel.Instance.open(
			'/crm/deal/details/'+clientId+'/',
...
BX.AbsenceCalendar.ExportExcel = function (data)
{
	console.log(Object.entries(data))
	let CSVFile = new Blob([data], {
		type: "text/csv;text/plain;charset=windows-1252",
	...
	link.href = url;
	const content = BX('bx_calendar_conrol_departments')
...
BX.AbsenceCalendar.tableToCsv = function (elements) {
