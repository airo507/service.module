...
JCCalendarViewMonth.prototype.__drawData = function()
{
	...
	for (var i = 0; i < (null == this.ENTRIES ? 0 : this.ENTRIES.length); i++)
	{
		...
		var strName = this.ENTRIES[i]['NAME'];
	...
	for (var i = 0; i < (null == this.ENTRIES ? 0 : this.ENTRIES.length); i++)
	{
		...
		if (obUserRow && this.ENTRIES[i]['DATA'])
		{
			...
			for (var j = 0; j < this.ENTRIES[i]['DATA'].length; j++)
			{
				...
				this.ENTRIES[i]['DATA'][j].VISUAL.style.background = "cyan";

				this.ENTRIES[i]['DATA'][j].VISUAL.innerHTML =
					'<nobr>'
					+ BX.util.htmlspecialchars(this.ENTRIES[i]['DATA'][j]['NAME'])
					+ ' (' + this.ENTRIES[i]['DATA'][j]['DATE_ACTIVE_FROM'] + ' - ' + this.ENTRIES[i]['DATA'][j]['DATE_ACTIVE_TO'] + ')'
					+ '</nobr>';
				...
				var dateStartNumber = date_start.valueOf() < ts_start.valueOf() ? ts_start.getDate() : date_start.getDate()
				var dateFinishNumber = date_finish.valueOf() < ts_finish.valueOf() ? date_finish.getDate() : ts_finish.getDate()

				var obStartCell = obUserRow.cells[dateStartNumber];
				var obFinishCell = obUserRow.cells[dateFinishNumber];
				...
				this.ENTRIES[i]['DATA'][j]['START_POS'] = start_pos
				this.ENTRIES[i]['DATA'][j]['FINISH_POS'] = finish_pos

				this._parent.MAIN_LAYOUT.appendChild(this.ENTRIES[i]['DATA'][j].VISUAL);
				this._parent.RegisterEntry(this.ENTRIES[i].DATA[j]);
			}
			var secondEntriesArray = this.ENTRIES[i]['DATA'];
			let errorWidth, positiveWidth

			for (let k = 0; k < this.ENTRIES[i]['DATA'].length; k++) {
				for (let m = 0; m < secondEntriesArray.length; m++) {
					if (secondEntriesArray[m]['ID'] !== this.ENTRIES[i]['DATA'][k]['ID']) {
						if (
							this.ENTRIES[i]['DATA'][k]['START_POS'] < secondEntriesArray[m]['FINISH_POS']
							&& this.ENTRIES[i]['DATA'][k]['START_POS'] > secondEntriesArray[m]['START_POS']
						) {
							errorWidth = secondEntriesArray[m]['FINISH_POS'] - this.ENTRIES[i]['DATA'][k]['START_POS'];
							positiveWidth = this.ENTRIES[i]['DATA'][k]['FINISH_POS'] - secondEntriesArray[m]['FINISH_POS'];
							this.ENTRIES[i]['DATA'][k].VISUAL.style.background = `linear-gradient(to right, red ${errorWidth}px, cyan ${positiveWidth}px)`
							break
						}
						if (
							this.ENTRIES[i]['DATA'][k]['FINISH_POS'] > secondEntriesArray[m]['START_POS']
							&& this.ENTRIES[i]['DATA'][k]['FINISH_POS'] < secondEntriesArray[m]['FINISH_POS']
						) {
							errorWIdth = this.ENTRIES[i]['DATA'][k]['FINISH_POS'] - secondEntriesArray[m]['START_POS'];
							positiveWidth = secondEntriesArray[m]['START_POS'] - this.ENTRIES[i]['DATA'][k]['START_POS'];
							this.ENTRIES[i]['DATA'][k].VISUAL.style.background = `linear-gradient(to right, cyan ${positiveWidth}px, red ${errorWIdth}px)`
							break
						}
